    ...
    <div style={styles.overlay}>
      <div style={styles.card}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 8 }}>
              {TRADING_STYLES.map(s => (
                <button
                  ...
                  style={{
                    ...styles.styleBtn,
                    borderColor: tradingStyle === s.key ? "var(--primary)" : "rgba(233,30,99,0.22)",
        ...
        {step === 2 && (
          <div style={styles.stepContent}>
            <p style={styles.bubble}>
              Goed. We beginnen met <strong style={{ color: "var(--primary)" }}>paper trading</strong> — dat is oefenen met nep-geld maar echte marktprijzen.
            </p>
            <p style={styles.bubble}>
              Je krijgt €10.000 startkapitaal. Maak fouten, leer ervan. Geen echt geld op het spel.
            </p>
            <div style={styles.capitalBox}>
              <span style={{ fontSize: 12, color: "var(--text-secondary)", textTransform: "uppercase", letterSpacing: "0.06em" }}>Jouw startkapitaal</span>
              <span style={{ fontSize: 32, fontWeight: 800, color: "var(--primary)" }}>€10.000</span>
            </div>
          </div>
        )}
        ...
        <div style={styles.dots}>
          {Array.from({ length: TOTAL_STEPS }).map((_, i) => (
            <div key={i} style={{ ...styles.dot, background: i === step ? "var(--primary)" : "rgba(233,30,99,0.22)" }} />
          ))}
        </div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  card: { position: "relative", width: "100%", maxWidth: "460px", background: "var(--surface)", border: "1px solid rgba(233,30,99,0.25)", borderRadius: "16px", padding: "32px 28px 24px", boxShadow: "0 32px 80px rgba(0,0,0,0.7)" },
  ...
  name: { fontWeight: 700, fontSize: 16, color: "var(--text)" },
  role: { fontSize: 12, color: "var(--text-secondary)" },
  bubble: { background: "rgba(233,30,99,0.06)", border: "1px solid rgba(233,30,99,0.15)", borderRadius: "0 12px 12px 12px", padding: "12px 16px", fontSize: 14, color: "var(--text)", lineHeight: 1.6, margin: 0 },
  ...
  assignment: { background: "rgba(233,30,99,0.08)", border: "1px solid rgba(233,30,99,0.3)", borderRadius: "10px", padding: "16px", fontSize: 14, color: "var(--text)", lineHeight: 1.6 },
  ctaRow: { display: "flex", gap: "12px", marginTop: "8px" },
  ctaPrimary: { flex: 1, background: "var(--primary)", color: "#fff", border: "none", borderRadius: "10px", padding: "13px 20px", fontSize: "16px", fontWeight: 700, cursor: "pointer" },
  ...
  navBtn: { background: "transparent", border: "1px solid rgba(233,30,99,0.3)", color: "var(--text-secondary)", borderRadius: "8px", padding: "9px 18px", fontSize: "14px", cursor: "pointer" },
  navBtnPrimary: { background: "var(--primary)", border: "none", color: "#fff", borderRadius: "8px", padding: "9px 18px", fontSize: "14px", fontWeight: 600, cursor: "pointer" },
};
