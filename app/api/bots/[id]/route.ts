...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const body = await request.json() as { name?: string; config?: Record<string, unknown>; active?: boolean; symbol?: string; simulation?: boolean };
  ...
  const simulation = body.simulation !== undefined ? (body.simulation ? 1 : 0) : (bot as unknown as { simulation: number }).simulation ?? 0;

  db.prepare(
    "UPDATE bots SET name = ?, config = ?, active = ?, symbol = ?, simulation = ?, updated_at = datetime('now') WHERE id = ?"
  ).run(name, config, active, symbol, simulation, parseInt(id));
